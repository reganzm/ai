...
if __name__ == '__main__':
    ...
    major_df_t = spark.read.csv('/data/datasets/salary_predict/v3/major.csv', header=True).toPandas()
    major_df_t.index = major_df_t['name']
    major_df_map = major_df_t.to_dict()

    school_df_t = spark.read.csv('/data/datasets/salary_predict/v3/university.csv', header=True).select('sid',
                                                                                                        'name').toPandas()
    school_df_t.index = school_df_t['name']
    school_df_map = school_df_t.to_dict()

    degree_df_t = spark.read.csv('/data/datasets/salary_predict/v3/degree.csv', header=True).toPandas()
    ...
    def func_major_hot(name):
        if major_df_map['name'].get(name):
            return int(major_df_map['hot'].get(name)) * 2
        else:
            return int(major_df_map['hot'].get('unknown')) + 1


    # 高薪权重为3
    def func_major_high(name):
        if major_df_map['name'].get(name):
            return int(major_df_map['high'].get(name)) * 3
        else:
            return int(major_df_map['high'].get('unknown')) + 1
    ...
    def work(spark):
        spark.read.parquet('/user/hdfs/resume_last_educations.parquet').createOrReplaceTempView('A')
        s_sql = """                 
            select 
            A.*,
            func_degree(degree) as degree_code,
            func_degree_w(degree) as degree_w,
            func_school(school) as school_code, 
            func_major(major) as major_code,
            func_major_high(major) as major_high,
            func_major_hot(major) as major_hot 
            from A
        """
        spark.sql(s_sql).repartition(100).write.mode('overwrite').parquet(
            'resume_educations_with_codes_v3.parquet')
