...
if __name__=="__main__":
    spark  = SparkSession.builder.master("yarn").appName("resume_works_coding_and_zhimeli").config("spark.ui.port", "44040")\
    ...
    def work(spark):
        ...
        select 
           ...
           get_center(B.province,B.city) as city_w,
           get_area(B.province) as area_w
        from 
           A inner join B 
           on A.resume_id=B.resume_id 
           and A.`max(index)`=B.index 
        ...
        df_salary.repartition(100).write.mode('overwrite').parquet('/user/bigdata/BI/resume_flatten_v1_20180813/resume_works_zhimeli_locations_v2.parquet')        
        
    province_df_t = spark.read.csv('/data/datasets/salary_predict/v2/province.csv',header=True).toPandas()
