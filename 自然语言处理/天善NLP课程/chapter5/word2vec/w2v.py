...
#replace参数为False表示每次运行采样结果不变，若replace参数设置为True，每次运行采样函数返回的结果都会发生变化
#从range(window)中采集出valid_size个结果用于验证
...
with graph.as_default():
    ...
    #embedding_lookup，从embeddings矩阵中查找出行号为train_inputs中指定的行号，返回这些行号对应在embeddings矩阵中所有的行然后组成的矩阵
    embed = tf.nn.embedding_lookup(embeddings, train_inputs)

    #计算噪声对比度估计损失：noise contrastive estimation
