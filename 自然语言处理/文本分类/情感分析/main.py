...
for i in range(train_iterations):
  ...
  sess.run(optimizer, feed_dict={input_data: next_batch, labels: next_batch_labels})
  if i % 10 == 0:
    print("正确率:%s  迭代次数:%s 损失值:%s"%(sess.run(accuracy, {input_data: next_batch, labels: next_batch_labels}),i,sess.run(loss, {input_data: next_batch, labels: next_batch_labels})))
...
for i in range(iterations):
  ...
  print("预测正确率:", (sess.run(accuracy, {input_data: next_batch, labels: next_batch_labels})) * 100)
